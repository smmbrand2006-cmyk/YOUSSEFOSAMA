...
### 7) إصلاح تنبيه "استنى ثواني لحد ما الحساب يتحمّل" وفك قفل المودال:
- **المشكلة السابقة:** كان المودال يتحقق بشرط `if (!userProfile?.uid)` ويظهر تنبيه `alert("استنى ثواني لحد ما الحساب يتحمّل وحاول تاني")`. هذا الشرط كان يسبب مأزقاً كاملاً (Deadlock) للزائرين الجدد أو المستخدمين في صفحة تسجيل الدخول؛ لأن المستخدم ليس مسجلاً بعد، وبالتالي `userProfile` ستظل `null` للأبد، وكلما ضغط على الزر يظهر نفس التنبيه ولا يمكنه منح الإذن أو الدخول للموقع!
- **الحل المطبق:**
  1. السماح بطلب أذونات المتصفح (الإشعارات والميكروفون) فوراً عند الضغط دون اشتراط وجود حساب، لأن المتصفح يقبل طلب الإذن لأي زائر.
  2. في حال كان المستخدم مسجلاً وموثقاً في Firebase (`auth.currentUser`)، يتم تسجيل التوكن في Firestore فوراً.
  3. في حال لم يكن مسجلاً بعد، يتم حفظ تأكيد الإذن في `localStorage` وإغلاق المودال بسلاسة، ويتولى `PWAClientManager` تسجيل وحفظ الـ FCM Token في Firestore تلقائياً وصامتاً بمجرد إتمام تسجيل الدخول أو إنشاء الحساب.
  4. إضافة زر رابط أسفل المودال: `"المتابعة لتسجيل الدخول / استخدام التطبيق"` لضمان عدم حجب الشاشة نهائياً تحت أي ظرف.
